inline void rhscal_auxvar(const side **psid, const int *aele, const elem *pele, uvar *pvar, double *pvar_grad)
{
		...
		for (int k = 0; k < NDIM; k++)
			for (int i = 0; i < nfvdof; i++)
			{
				...
				for (int j = 0; j < NEQ; j++)
					...
					pvar_grad[k * 40 + i * 4 + j] += -tcv[j] * basxy[k];
			} // end of nfvdof
	}		  // element gauss quad
			...
			for (int k = 0; k < NDIM; k++)
				for (int i = 0; i < nfvdof; i++)
				{
					...
					for (int j = 0; j < NEQ; j++)
					{
						...
						pvar_grad[k * 40 + i * 4 + j] += temp2 * norm[k];
					} // each variables
				}
		} // each quad points
